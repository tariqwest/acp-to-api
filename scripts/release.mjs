#!/usr/bin/env node
/**
 * Create a GitHub release for acp-to-api, optionally publish to npm, and
...
const FORMULA_NAME = "acp-to-api.rb";
const FORMULA_CLASS_DEFAULT = "AcpToApi";
...
function publishHomebrewFormula({ version, tap, dryRun }) {
  ...
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "acp-to-api-brew-"));
  ...
  run("git", ["-C", tapDir, "commit", "-m", `formula(acp-to-api): ${version}`]);
  ...
  console.log(`Install: brew tap ${owner}/tap && brew install acp-to-api`);
}
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    if (opts.homebrew) {
      ...
      console.log(`Homebrew: brew tap ${owner}/tap && brew install acp-to-api`);
